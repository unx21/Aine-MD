...
klik wa.me/${conn.user.jid.split`@`[0]}`, m.chat, 
      ...
      if (!room.pilih) this.sendButton(room.p, `Silahkan pilih\n\nMenang +Rp${room.poin}\nKalah -Rp${room.poin_lose}`, wm, null, [[ 'Batu', 'Batu'], ['Kertas', 'Kertas'], ['Gunting', 'Gunting']], m)
      ...
      room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) this.reply(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`)
          ...
          db.data.users[win == room.p ? room.p : room.p2].uang += room.poin
          db.data.users[win == room.p ? room.p2 : room.p].uang -= room.poin_lose
        }
        delete this.suit[room.id]
        return !0
      }, room.timeout)
